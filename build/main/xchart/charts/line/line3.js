...
const drawCurve2 = (ctx, points, w, h, padding, colors = ['#FF0000', '#00FF00']) => {
    ...
    ctx.quadraticCurveTo(points[i + 1].x, points[i + 1].y, w - padding.right, h - padY);
...
const lineChart3 = (app, data, options = types_1.defaultLineChartOptions) => {
    return (instance) => {
        var _a, _b;
        const xValues = (0, range_1.rangeToArray)(((_a = options.xAxis) === null || _a === void 0 ? void 0 : _a.range) || []);
        const yValues = data.values || (0, range_1.rangeToArray)(((_b = options.yAxis) === null || _b === void 0 ? void 0 : _b.range) || []);
        ...
        let points = items.map((item, i) => {
            return (0, vector_1.VEC2)(padding + i * spaceBetween, scaleY(item.value));
        });
        drawCurve2(ctx, points, size.x, size.y, { left: padding, right: padding, top: 0, bottom: padding }, options.colors || ['#ff0000', '#00ff00']);
        //const SMOOTH = true;
        //if (!SMOOTH) {
        //ctx.strokeStyle = "#000";
        //ctx.beginPath();
        ...
        items.forEach(function (point, index) {
            if (index % labelInterval === 0) {
                const getText = () => {
                    if ((0, is_1.isNumber)(point.date))
                        return `${point.date}`;
                    const date = new Date(point.date);
                    const formattedDate = `${date.getMonth() + 1}/${date.getDate()}/${date.getFullYear()}`;
                    return formattedDate;
                };
                ctx.fillText(getText(), padding + index * spaceBetween, size.y - padding + 20);
            }
        });
    };
};
exports.lineChart3 = lineChart3;
