...
export const donutChart: ChartRunFunction = (
  ...
  const segments: DonutSegment[] = total <= 0.0000000001 ? [] : data.values.map((item, i) => {
  ...
  if (needsQuirk) {
    ...
    segments.forEach(function (p, i) {
      ...
      const s1 = clamp(
        smoothstep(instance.size.y * 0.5, instance.size.y * 0.01, mouseDist) +
      ...
      let s = smin(
        sgt(mangle, startAngle, 0.5),
        slt(mangle, endAngle, 0.5),
        0.5,
      ); //(mangle >= startAngle && mangle <= endAngle) ? 1.0 : 0.0;//Math.ceil(Math.max(0.0, 1.0 - Math.abs(sliceAngle - Math.sin(TAU*Math.acos(dot))))-0.5);
      ...
      s *= smoothstep(
        instance.size.y * 0.75,
        instance.size.y * 0.15,
        mouse.distance(center) * 0.95,
      );
  ...
  const updateTooltip = (instance: ChartInstance) => {
    ...
    instance.tooltip.state.opacity = needsQuirk ? 0 : Math.max(
