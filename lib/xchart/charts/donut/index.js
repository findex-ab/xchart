...
export var donutChart = function (app, center, data, options, callback) {
    ...
    var getSegmentDistances = function (seg) {
        var mouseAngle = getMouseAngle(app.mouse, center);
        var dist = seg.pos.distance(app.mouse);
    ...
    var getHoveredSegment = function (segments) {
        if (app.mouse.distance(center) > app.size.y * 1.5)
            return;
    ...
    segments.forEach(function (p, i) {
        ...
        var mouse = app.mouse
            .mul(VEC2(1.0 / app.size.x, 1.0 / app.size.y))
            .mul(VEC2(app.size.x, app.size.y));
        var mangle = getMouseAngle(mouse, center);
        var mouseDist = mouse.distance(p.pos);
        var s1 = clamp(smoothstep(app.size.y * 0.5, app.size.y * 0.01, mouseDist) +
            0.25 * app.invMouseDistance, 0.0, 1.0);
        ...
        if (segments.length <= 1) {
            ...
            if (hovered && app.mouse.distance(center) < app.size.x * 0.5) {
                s = 1.0;
            }
        }
        ...
        s *= smoothstep(app.size.y * 0.75, app.size.y * 0.15, mouse.distance(center) * 0.95);
...
export var donutChart__backup = function (app, center, data, options, callback) {
    if (options === void 0) { options = defaultDonutOptions; }
    var ctx = app.ctx;
    var state = {
        activeSegment: undefined
